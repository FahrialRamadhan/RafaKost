    ...
    <div class="page-wrapper">
        <div class="content-container">
            ...
            <div style="margin-bottom: 32px; display: flex; gap: 8px; align-items: center;">
                ...
                <h1 style="font-size: 16px; font-weight: 700; color: #000000; margin: 0;">Edit Kamar</h1>
            </div>
            ...
            <form action="{{ route('kamars.update', $kamar->id) }}" method="POST" enctype="multipart/form-data">
				...
				<div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 24px; margin-bottom: 20px;">
				    <div>
				        ...
				        <input type="text" name="nama" class="form-input" placeholder="Cth.Kamar A1" value="{{ old('nama', $kamar->nama) }}">
				        @error('nama') <p style="color: #e53e3e; font-size: 11px; margin-top: 6px;">{{ $message }}</p> @enderror
				    </div>
				
				    <div>
				        <label class="form-label">Lantai</label>
				        <input type="text" name="lantai" class="form-input" placeholder="Cth.Lantai 1" value="{{ old('lantai', $kamar->lantai) }}">
				    ...
				    <div>
				        ...
				        <input type="text" name="kamar_mandi" class="form-input" placeholder="Cth. dalam / luar" value="{{ old('kamar_mandi', $kamar->kamar_mandi) }}">
				    ...
				    <div>
				        ...
				        <div style="position: relative;">
				            ...
				            <input type="number"
				                   ...
				                   value="{{ old('harga_1_orang', $kamar->harga_1_orang ?? $kamar->harga) }}"
				    ...
				    <div>
				        ...
				        <div style="position: relative;">
				            ...
				            <input type="number"
				                   ...
				                   value="{{ old('harga_2_orang', $kamar->harga_2_orang ?? $kamar->harga) }}"
				                   style="padding-left:44px;">
				        </div>
				...
				<div style="margin-bottom: 20px;">
				    ...
				    <textarea
				        ...
				        rows="5"
				        class="form-input"
				        placeholder="Contoh: Kamar nyaman, bersih, cocok untuk mahasiswa atau pekerja. Sudah termasuk kasur, meja, lemari, dan akses fasilitas umum.">{{ old('description', $kamar->description) }}</textarea>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
                        <label style="cursor: pointer; display: block;">
                            <input type="radio" name="status" value="tersedia" {{ old('status', $kamar->status) == 'tersedia' ? 'checked' : '' }} style="display: none;" class="status-radio">
                            <div class="status-card {{ old('status', $kamar->status) == 'tersedia' ? 'active' : '' }}">
                                <div class="radio-circle"></div>
                                <span style="color: #000000; font-size: 13px; font-weight: 700;">Tersedia</span>
                            </div>
                        </label>
                        <label style="cursor: pointer; display: block;">
                            <input type="radio" name="status" value="terisi" {{ old('status', $kamar->status) == 'terisi' ? 'checked' : '' }} style="display: none;" class="status-radio">
                            <div class="status-card {{ old('status', $kamar->status) == 'terisi' ? 'active' : '' }}">
                                <div class="radio-circle"></div>
				...
				<div style="margin-bottom: 20px;">
				    ...
				    @if($kamar->image)
				        <div style="display: flex; align-items: center; gap: 12px; padding: 12px; border: 1px solid #E2E8F0; border-radius: 8px; margin-bottom: 12px;">
				            <img src="{{ asset('storage/' . $kamar->image) }}" alt="Foto" style="width: 48px; height: 48px; object-fit: cover; border-radius: 6px;">
				            <span style="font-size: 12px; color: #475569;">Foto utama saat ini terpasang.</span>
				        </div>
				    @endif
				    ...
				    <label for="image-upload" style="display: block;">
				        <div class="upload-area">
				            ...
				            <p id="upload-text" style="color: #000000; font-size: 13px; font-weight: 700; margin: 0 0 4px;">Klik Unggah Foto Utama Baru (Opsional)</p>
				    ...
				    <input type="file" name="image" id="image-upload" accept="image/*" style="display: none;"
				           onchange="document.getElementById('upload-text').textContent = this.files[0] ? this.files[0].name : 'Klik Unggah Foto Utama Baru (Opsional)'">
				...
				@php
				    ...
				    if (!empty($kamar->images)) {
				        $decodedGalleryImages = json_decode($kamar->images, true);
				        $existingGalleryImages = is_array($decodedGalleryImages) ? $decodedGalleryImages : [];
				    }
				@endphp
				        ...
				        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(90px,1fr));gap:10px;">
				            @foreach($existingGalleryImages as $galleryImage)
				                <div style="border:1px solid #E2E8F0;border-radius:8px;padding:6px;">
				                    <img src="{{ asset('storage/' . $galleryImage) }}" alt="Gallery" style="width:100%;height:80px;object-fit:cover;border-radius:6px;">
				                </div>
				            @endforeach
				        </div>
				    </div>
				...
				<div style="margin-bottom: 32px;">
				    <label class="form-label">Tambah Foto Gallery</label>
				    <label for="images-upload" style="display: block;">
				        <div class="upload-area">
				            ...
				            <p style="color: #94A3B8; font-size: 11px; margin: 0;">Foto baru akan ditambahkan ke gallery lama.</p>
				        </div>
				    </label>
